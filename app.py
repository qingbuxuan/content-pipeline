...
## 写作要求
1. 字数：1000-1200字，结尾附5个话题标签 #话题标签
2. 去结构化：口语化，不用1、2、3排序
...
**第三把钩子**：结尾让人行动/收藏/转发——可操作步骤+亲身经历+金句收尾+互动提问（留言）
...
### 1. 核心主体（选1-2个）
- 具体人物（如：华人、一位满头银发的老人、穿着白大褂的中年医生）
...
### 6. 风格定位
- 摄影风格：电影感纪实、杂志封面、复古胶片、忧郁、史诗感
- 插画风格：扁平插画、水墨国风、治愈系插画
- 混合风格：光影插画、拼贴艺术

### 7. 景别与视角
- 景别：大远景、全景、中景、特写、大特写
- 视角：平视、低角度、高角度、鸟瞰、虫眼、第一人称视角

### 8. 环境细节
- 晨雾笼罩（shrouded in morning mist）
- 飘落的樱花（falling cherry blossoms）
- 背景虚化（background bokeh）

### 9. 人脸保真
- 画质：4K超高清，电影质感，细节丰富
- 稳定约束：面部清晰不变形，人体结构正常，动作流畅不僵硬。高清肤质，细致毛孔，真实光影，原生皮肤
- 风格：真实质感，真人出镜
...
👆 以上是今日生成的文章内容"""
    ...
    if cover_url:
        ...
        # 构建完整文章（文首固定标识 + 正文 + 文末预告 + 引导关注）
        banner = WEEKLY_BANNERS.get(weekday, {})
        next_wk = (weekday + 1) % 7
        next_banner = WEEKLY_BANNERS.get(next_wk, {})
        article_full = (
            f"{banner.get('icon', '')} **{WEEKDAY_NAMES[weekday]} · {theme_info.get('name', '')}**"
            f" 「{banner.get('column', '')}」 {banner.get('slogan', '')}\n\n"
            + article
            + f"\n\n---\n\n"
        ...
        article_html = markdown_to_html(article_full, weekday)
